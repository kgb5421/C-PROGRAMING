...
int main() {
    ...
    char productNames[MAX_PRODUCTS][100] = {"", "", "", "", ""}; // 상품명 저장
    ...
    while (1) {
        ...
        if (choice == 1) {
            ...
            for (int i = 0; i < MAX_PRODUCTS; i++) {
                printf("ID %d 상품 (%s) - 입고 수량: %d, 판매 수량: %d, 남은 재고: %d\n", 
                        i + 1, productNames[i], stock[i], sold[i], remainingStock[i]);
            ...
            for (int i = 0; i < MAX_PRODUCTS; i++) {
                if (stock[i] > 0) {
                    ...
                    printf("ID %d 상품 (%s) - 판매율: %.2f%%\n", i + 1, productNames[i], salesRate);
                }
            }
            ...
            printf("\n가장 많이 판매된 상품: ID %d (%s), 판매 수량: %d\n", maxSoldID, productNames[maxSoldID - 1], maxSold);
            printf("가장 적게 판매된 상품: ID %d (%s), 판매 수량: %d\n", minSoldID, productNames[minSoldID - 1], minSold);

            // 재고 부족 상품 한 번에 출력
            printf("\n재고 부족 상품 (남은 재고 2 이하):\n");
            for (int i = 0; i < MAX_PRODUCTS; i++) {
                if (remainingStock[i] <= 2) {
                    printf("ID %d (%s) 상품은 재고 부족 (남은 재고: %d)\n", i + 1, productNames[i], remainingStock[i]);
                }
            }
            ...
            for (int i = 0; i < MAX_PRODUCTS; i++) {
                printf("ID %d 상품명: ", i + 1);
                scanf_s("%s", productNames[i], sizeof(productNames[i]));
            }
        }
        else if (choice == 5) {
            // 5. 종료
